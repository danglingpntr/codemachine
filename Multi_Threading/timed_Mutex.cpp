...
void increment(int i) 
{
    ...
    // if(m.try_lock_until(now + std::chrono::seconds(2)))          // Both for and until works for same time duration
    // {
    if(m.try_lock_for(std::chrono::seconds(1)))              //if this time is less then the time taken by thread
    {                                                        //inside the lock then 2nd thread can not enter,swap them to check
        ++myAmount;                                                 
